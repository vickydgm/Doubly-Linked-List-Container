/* Name: Xiangzhen Sun
   Projetc: #2
   Description: this is an implementation of template class List<T>, imitating the C++11 STL container double-lined list
   Besides, the template class along with its nested class members are integrated in a namespace for later utilization
   on .cpp files. It is worthwhile to mention that the program runs perfectly. Under the help of online resources and shared
   doubly-linked list container source program, I am finally able to assembly this STL container !
*/
